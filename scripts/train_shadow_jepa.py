"""
Training script for the paper-faithful shadow JEPA stage.

Learns:
    encoder(scene_t) -> z_t
    predictor(z_t, sun_{t+1} - sun_t) -> z_{t+1}

Loss:
    MSE(z_pred, z_target) + lambda * SIGReg(z)
"""
...
from shadow_prediction.model_shadow_world import create_shadow_jepa_model


class ShadowJEPATrainer(BaseTrainer):
    """Trainer for the decoder-free shadow JEPA/world-model stage."""

    def __init__(self, latent_dim=128, base_channels=32, sigreg_weight=0.1,
                 sequence_length=4, random_projection=False, **kwargs):
        kwargs.setdefault("task_name", "Shadow JEPA Stage-1 Training")
    ...
    def create_model(self):
        model, _ = create_shadow_jepa_model(
    ...
    def get_extra_info_lines(self):
        return [
            ...
            "State: shadowed scene; Action: sun_{t+1} - sun_t; Target: next latent",
        ]
